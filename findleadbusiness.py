        ...
        while True:
            try:
                ...
                for line in litext.splitlines():
                    ...
                    searchEmails = re.findall(r"[a-zA-Z]+[a-zA-Z0-9\.\-\_]+@{}".format(valableLink),line, flags=re.MULTILINE)
...
def whriteInOuput(finalOutput):
    ...
    os.chdir("D:/IIHT/Python/Project/Find lead business/groupon.com/Ville et nom testé/Hartwell")
    lenEmail = len(mails)
    ...
    with open("{}---{}.txt".format(search,location), "w") as writ:
...
for link in domainName :
    ...
    if not re.findall('facebook',valableLink):
        if valableLink not in valableLinktable:
            valableLinktable.append(valableLink)

for valableLink in valableLinktable:
